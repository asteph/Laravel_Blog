...
@section('content')
    ...
    @forelse($posts as $post)
        ...
        <p><span class="glyphicon glyphicon-time"></span>  {{{$post->created_at->setTimezone('America/Chicago')->format('l, F jS Y @ h:i:s A')}}}  by <a href="/?user={{$post->user_id}}">{{ $post->user->first_name }} {{ $post->user->last_name }}</a></p>
        <hr>
